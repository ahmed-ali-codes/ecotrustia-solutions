...
import { FaFilePdf, FaFileWord, FaExchangeAlt, FaDownload, FaRedo, FaShieldAlt, FaBolt, FaLayerGroup, FaCheck } from 'react-icons/fa';
...
const PdfToWord: React.FC = () => {
    ...
    const handleFileSelect = (selectedFile: File) => {
        if (selectedFile && selectedFile.type === 'application/pdf') {
            ...
            alert("Invalid Protocol: Please upload a valid PDF document.");
        }
    };
    ...
    return (
        <div className="min-h-screen bg-[#050505] text-white selection:bg-blue-500/30 font-sans">
            <title>PDF to Word Transposer | Ecotrustia Solutions</title>
            <meta name="description" content="Convert static PDF documents into editable Word architectures. Maintain structural integrity and textual flow across document transpositions." />
            ...
            <section className="relative pt-40 pb-24 px-[5%] overflow-hidden flex flex-col items-center justify-center min-h-[50vh]">
                ...
                <div className="relative z-10 max-w-5xl w-full mx-auto flex flex-col items-center text-center">
                    <h1 className="text-5xl md:text-8xl lg:text-9xl font-black mb-8 tracking-[-0.05em] leading-[0.9] text-white uppercase text-glow">
                        PDF to Word <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-500">Converter</span>
                    </h1>
                    <p className="text-gray-400 text-lg md:text-xl max-w-3xl leading-relaxed font-medium italic">
                        "Convert static document nodes into editable linguistic architectures with absolute textual fidelity."
                    </p>
                </div>
            </section>
                ...
                <div className="max-w-4xl w-full mx-auto">
                    ...
                    <div className="glass-workbench shadow-2xl relative" style={{ padding: 'clamp(24px, 5vw, 48px)', marginBottom: '120px' }}>
                        {!file ? (
                            ...
                            >
                                ...
                                <p className="text-gray-500 text-sm font-medium mb-2">Drag & drop your PDF document or click to explore local nodes</p>
                                <span className="mt-6 text-[9px] uppercase tracking-[0.2em] text-gray-600 font-bold">Supported Protocol: PDF (Standard Architecture)</span>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                                    ...
                                    <div className="p-8 rounded-[32px] bg-white/5 border border-white/10 flex flex-col gap-8">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="flex flex-col truncate">
                                                ...
                                                <span className="text-[10px] text-gray-500 font-black uppercase tracking-widest">{(file.size / 1024).toFixed(1)} KB Source Node</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="p-6 rounded-2xl bg-black/40 border border-white/5 flex flex-col items-center justify-center text-center gap-4 py-12">
                                            ...
                                            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-600">Architectural Re-Mapping</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-col justify-center gap-8">
                                    {!showResult ? (
                                        ...
                                        >
                                            {isConverting ? <span className="animate-pulse">Synthesizing Editable Grid...</span> : <>Initialize Transposition <FaFileWord className="text-sm" /></>}
                                        </button>
                                    ) : (
                                        <div className="space-y-4 animate-in zoom-in duration-500">
                                            ...
                                            >
                                                <FaDownload /> Download Editable Protocol
                                            </button>
                                        </div>
                                    )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-12" style={{ marginBottom: '120px' }}>
                        <div className="mini-card group flex flex-col gap-5">
                            ...
                            <h5 className="font-bold text-sm text-white">Structural Sync</h5>
                            <p className="text-[11px] text-gray-500 leading-relaxed italic">Preserve complex paragraph layouts and font mappings during the architectural transposition.</p>
                        </div>
                        <div className="mini-card group flex flex-col gap-5">
                            <FaBolt className="block text-indigo-400 text-xl group-hover:scale-125 group-hover:-translate-y-1 transition-transform" />
                            <h5 className="font-bold text-sm text-white">Neural Mapping</h5>
                            <p className="text-[11px] text-gray-500 leading-relaxed italic">Process textual data into editable Word buffers with high-velocity algorithmic logic.</p>
                        </div>
                        <div className="mini-card group flex flex-col gap-5">
                            <FaShieldAlt className="block text-blue-400 text-xl group-hover:scale-125 group-hover:-translate-y-1 transition-transform" />
                            <h5 className="font-bold text-sm text-white">Isolated Buffer</h5>
                            <p className="text-[11px] text-gray-500 leading-relaxed italic">Document arithmetic is restricted to the localized client node. Zero external data leakage.</p>
                        </div>
                    </div>

                    {/* Intelligence Section */}
                    <div className="about-workbench-section text-center" style={{ marginBottom: '120px' }}>
                        <h2 className="text-3xl font-black tracking-tight text-white font-sans text-center uppercase tracking-widest" style={{ marginBottom: '20px' }}>Document Intelligence</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 font-sans max-w-4xl mx-auto text-gray-500">
                            <div className="faq-item border-l border-white/5 pl-8 text-left">
                                <h5 className="font-bold mb-4 text-white uppercase text-xs tracking-widest">Will my images be preserved?</h5>
                                <p className="text-sm leading-relaxed">Yes. Our transposition engine identifies visual asset nodes within the PDF and maps them directly into the Word document architecture with zero-loss quality retention.</p>
                            </div>
                            <div className="faq-item border-l border-white/5 pl-8 text-left">
                                <h5 className="font-bold mb-4 text-white uppercase text-xs tracking-widest">Are encrypted PDFs supported?</h5>
                                <p className="text-sm leading-relaxed">Encrypted document protocols require authorization for induction. Please ensure the source PDF is decrypted before attempting architectural re-mapping.</p>
                            </div>
                        </div>
                    </div>

                    <div style={{ marginBottom: '120px' }}>
                        <ExploreTools currentPath="/tools/pdf-to-word-converter" />
                    </div>
                </div>
            </section>
